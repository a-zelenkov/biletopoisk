...
import { selectAllTicketAmount } from '@/services/cart/selector';

export function Header() {
    const ticketAmount = useAppSelector(state => selectAllTicketAmount(state));

    return (
        <header className={classNames('primary', style.header)}>
            <Link href="/">
                <h1>Билетопоиск</h1>
            </Link>
            <Link href="/cart">
                <div className={style.cart}>
                    {ticketAmount > 0 && <div className={style.amount}>{ticketAmount}</div>}
                    <svg
